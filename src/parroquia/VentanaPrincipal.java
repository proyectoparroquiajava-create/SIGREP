...
public class VentanaPrincipal extends JFrame {
    ...
    private void construirSeccionTablaPersonas() {
        ...
        modeloTabla = new DefaultTableModel(
                new Object[] { "Rol", "Nombre", "DNI", "Telefono", "Dirección" }, 0) {
            @Override
    ...
    private void construirSeccionInscripcion() {
        ...
        comboTipoSacramento = new JComboBox<>(
                new String[] { "Bautizo", "Matrimonio", "Retiro", "Primera Comunión", "Confirmación" });
        ...
        comboBeneficiario.setBounds(120, 281, 200, 20);
        ...
        txtCosto.setBounds(140, 312, 100, 17);
        contentPane.add(txtCosto);

        JLabel lblFecha = new JLabel("Fecha (AAAA-MM-DD):");
        lblFecha.setBounds(10, 335, 110, 14);
        contentPane.add(lblFecha);

        txtFecha = new JTextField();
        txtFecha.setBounds(140, 332, 100, 20);
        ...
        txtHora.setBounds(140, 357, 100, 20);
        contentPane.add(txtHora);

        lblCupos = new JLabel("Cupos totales:");
        lblCupos.setBounds(10, 380, 120, 14);
        contentPane.add(lblCupos);

        txtCupos = new JTextField("40");
        txtCupos.setBounds(140, 377, 100, 19);
        contentPane.add(txtCupos);

        lblCuposDetalle = new JLabel("");
        lblCuposDetalle.setBounds(250, 380, 200, 14);
        ...
        btnActualizarCombos.setBounds(10, 405, 129, 20);
        ...
        btnCrearInscripcion.setBounds(150, 405, 190, 20);
    ...
    private void alRegistrarPersona(ActionEvent evento) {
        try {
            ...
            modeloTabla.addRow(new Object[] {
                    rol,
                    nuevaPersona.getNombreCompleto(),
                    dni,
                    telefono,
                    direccionCargo
            });
    ...
    private void alModificarPersona(ActionEvent evento) {
        ...
        String nuevoNombre = JOptionPane.showInputDialog(
                this, "Nombre completo:", nombreActual);
        ...
        String nuevoTelefono = JOptionPane.showInputDialog(
                this, "Teléfono:", telefonoActual);

        if (nuevoTelefono == null) {
            return;
        }

        String nuevaDireccion = JOptionPane.showInputDialog(
                this, "Dirección:", direccionActual);
        ...
        try {
            ...
            if ("Feligres".equals(rolActual)) {
                personaActualizada = new Feligres(
            ...
            personasPorClave.put(
                    claveDe(personaActualizada),
                    personaActualizada);
    ...
    private void alCrearInscripcion(ActionEvent evento) {
        try {
            Persona beneficiario =
                    personasPorClave.get(
                            (String) comboBeneficiario.getSelectedItem());

            if (beneficiario == null) {
                throw new DatosInvalidosException(
                        "Debe registrar y seleccionar un beneficiario.");
            }

            Persona personaSacerdote =
                    personasPorClave.get(
                            (String) comboSacerdote.getSelectedItem());
            ...
            if (sacramento instanceof Retiro) {
                texto += "\nCupos disponibles en esa fecha: "
                        ...
                        + " de "
                        + ((Retiro) sacramento).getCuposTotales();
            }
            ...
            JOptionPane.showMessageDialog(this,
                    "El costo y los cupos deben ser numeros validos.",
                    "Error de formato",
                    JOptionPane.ERROR_MESSAGE);
    ...
    private Sacramento crearSacramentoSegunTipo(
        ...
        if ("Bautizo".equals(tipoUi)) {
            Persona padrino =
                    personasPorClave.get(
                            (String) comboCampoExtra1.getSelectedItem());

            Persona madrina =
                    personasPorClave.get(
                            (String) comboCampoExtra2.getSelectedItem());
            ...
            if (sacerdote.equals(padrino)
                    || sacerdote.equals(madrina)) {
        ...
        if ("Matrimonio".equals(tipoUi)) {
            ...
            Persona testigo2 =
                    personasPorClave.get(
                            (String) comboCampoExtra3.getSelectedItem());
    ...
    private int cuposTotalesParaRetiro(LocalDate fecha) {
        Integer capacidadExistente =
                repositorio.capacidadRetiroEn(fecha);
    ...
    private void alGenerarReportePorTipo(ActionEvent evento) {
        ...
        Reportes.ReporteTipo reporte =
                Reportes.generarReportePorTipo(
        ...
        texto.append("Reporte de ")
                .append(tipoUi.toLowerCase())
                .append(" - cantidad: ")
                .append(reporte.cantidad())
                .append("\n");
        ...
        texto.append("Total recaudado en ")
                .append(tipoUi.toLowerCase())
                .append(": S/ ")
    ...
    private void alCalcularTotal(ActionEvent evento) {
        ...
        areaResultado.setText(
                String.format(
                        "Total recaudado (todas las inscripciones): S/ %.2f",
                        total));
    }
    ...
    private void alGuardarRespaldo(ActionEvent evento) {
        try {
            var ruta =
                    repositorio.guardarRespaldo(
                            "respaldo_parroquia.txt");
    ...
    private void actualizarCamposSegunTipo() {
        ...
        boolean esMatrimonio =
                "Matrimonio".equals(tipo);

        boolean esRetiro =
                "Retiro".equals(tipo);

        boolean esPrimeraComunion =
                "Primera Comunión".equals(tipo);

        boolean esConfirmacion =
                "Confirmación".equals(tipo);
    ...
    private void actualizarCuposDisponibles() {
        if (comboTipoSacramento == null
                || !"Retiro".equals(
                        comboTipoSacramento.getSelectedItem())) {
            return;
        }

        LocalDate fecha =
                fechaProgramadaDesdeFormulario();
        ...
        int ocupados =
                repositorio.cuposOcupadosRetiroEn(fecha);
